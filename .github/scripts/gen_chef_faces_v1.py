...
def sheet_prompt(who: str, exprs: list[str]) -> str:
    ...
    return (
        ...
        "넘지 않는다. 배경도 첨부 이미지의 배경을 그대로 쓴다. "
        # ⚠ 260726 사고 재발 방지 — 여기에 **아트 스타일 지시를 절대 쓰지 마라**.
        # "라인아트"·"셀 채색"·"수채 금지" 같은 문구를 넣었더니 모델이 그걸 따라 원본 화풍을 갈아엎었다.
        # 첨부 이미지가 화풍의 유일한 출처다. 바뀌어야 하는 건 표정뿐이다.
        "첨부 이미지의 그림체·채색 방식·붓질 질감·선 굵기·명암 처리·색감을 **1%도 바꾸지 마라**. "
        "같은 화가가 같은 날 같은 도구로 그린 것처럼 보여야 한다. 스타일을 재해석하거나 정리하지 마라. "
        "달라지는 것은 오직 얼굴의 표정뿐이다. "
        # 과장된 만화 기호(땀방울·핏대·눈 하이라이트 폭발 등)를 넣으면 원본과 붕 뜬다.
        "표정은 **원본 그림의 절제된 표현 범위 안에서만** 바꿔라 — 놀람·화남 같은 큰 감정도 "
        "만화적으로 과장하지 말고, 눈매·눈썹·입꼬리의 미세한 변화로만 표현한다. "
        "원본에 없던 기호(땀방울·홍조 빗금·효과선·이모지풍 표시)를 추가하지 마라. "
        "얼굴 비율·이목구비 모양·머리 형태는 원본과 완전히 동일해야 하며, 다시 그리지 말고 "
        "원본 얼굴을 그대로 두고 표정 근육만 움직인 것처럼 보여야 한다. "
        "해상도와 묘사 밀도도 원본과 같게 — 단순화하거나 뭉개지 마라. "
...
def pose_prompt(who: str, poses: list[str], cols: int, rows: int) -> str:
    ...
    return (
        ...
        # ⚠ 아트 스타일 지시 금지(260726 사고) — 첨부 이미지가 화풍의 유일한 출처다.
        "첨부 이미지의 그림체·채색·질감·선·명암·색감을 **1%도 바꾸지 마라**. 스타일을 재해석하지 마라. "
        "달라지는 것은 인물의 자세와 그 뒤 배경뿐이다. 배경은 한국·일본풍 목조 실내와 마당, "
...
def bg_prompt(who: str, themes: list[str], cols: int, rows: int) -> str:
    ...
    return (
        ...
        # ⚠ 아트 스타일 지시 금지(260726 사고) — 첨부한 캐릭터 그림의 화풍을 배경도 그대로 따른다.
        "첨부 이미지와 같은 화가가 그린 배경처럼, 그 그림의 채색 방식·질감·색감·명암을 그대로 계승해라. "
