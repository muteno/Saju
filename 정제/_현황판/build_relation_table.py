...
VERDICT_NOTE = {
    ...
    "인과": "«그래서 이렇게 된다»를 잇는 고리 (발현·귀결·적용) — 지금 73개뿐",
...
# L1은 «검증 대상»이 아니라 **검증의 바탕**이다. 종류별로 제 이름을 붙인다.
#   그냥 «미검증»으로 두면 849개가 마치 «못 믿을 것»처럼 보인다 — 정반대다.
L1KIND = {}
for _k in ("생", "극", "소속", "지장간", "구성", "음양", "비화", "롤업", "육친",
           # ★260726 «산출» — 신살 조견표(일간→지지). «입력에서 글자를 뽑는 표»라
           #   공리에 가깝다. 더 물으면 «왜 갑목의 천을귀인이 축미인가»가 되는데
           #   그건 고전 표(甲戊庚牛羊)라 명리 밖이다.
           "산출",
           # ★260726 «절차» — 강약 점수제(110점)·득령득지득시득세. 조견표와 같은 등급이다:
           #   «원국 8글자를 넣으면 숫자·판정이 나온다»는 계산 규칙이라 더 물을 데가 없다.
           #   ⚠단 «85점 이상이면 극신강»의 85는 엔진이 스스로 «표본 부족 잠정치»라 적었고,
           #     자리 배점은 엔진과 도화도레 판본이 **연지↔시지로 뒤바뀌어** 갈린다 → stance.
           "절차"):
    L1KIND[_k] = "공리"
