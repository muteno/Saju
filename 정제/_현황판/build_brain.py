...
def main():
    ...
    for e in 링크:
        ...
        관계.append({
            ...
            "효과": e.get("effect"),
            # ★260728 발현 «살» — 축자 근거(살축자·살para)와 «근거없음» 깃발.
            #   근거없음 = 코퍼스 실증 0인데 서술만 있는 간선(천을귀인→구설 등 6건) —
            #   앱은 이 깃발이 선 간선으로 **단정 문장을 만들면 안 된다.**
            "살축자": e.get("살축자"), "살para": e.get("살para"),
            "근거없음": e.get("근거없음"),
    ...
    # ★260728 — 백과사전 배선(운영자: "머지한거는 앱에 적용되는건가?").
    #   그전엔 백과가 볼트에만 가고 앱은 53자 요약을 받았다 = 두 벌 정의.
    #   백과 «무엇»이 있으면 그걸 정의 정본으로, 산출·갈림도 실어 앱이
    #   「어떻게 계산되나」·「어디가 갈리나」까지 답하게 한다.
    try:
        백과 = {r["concept"]: r for r in jl("백과.jsonl")}
    ...
    for c, meta in nodes.items():
        ...
        노드[c] = {
            ...
            "정의": (enc.get("무엇") or 정의.get(c, "")),
            "산출": enc.get("산출", ""), "갈림": enc.get("갈림", ""),
            "문단": N(c), "출처수": S(c),
        }
