...
#
# ★260728 조임 — O2 위원 적발: 옛 정규식이 **12개 시험 문장 중 10개를 통과**시켰다.
#   「좋다」는 잡는데 「좋은/좋아지면」은 놓치고(한국어 활용), 「고난·출세·명예·부정적」은
#   어휘 자체가 없었다. 피그잼 B갈래 16건을 붓기 전에 막아야 하는 구멍.
#   ⚠구조 서술은 살린다 — 「기운이 흩어지면」(설기)·「합이 풀리면」(해소)은 결론이 아니다.
#     그래서 「흩어지」는 「재물이 흩어」로, 「풀리」는 「잘/안 풀리」로만 묶는다.
#   반례 = 검사_반례.jsonl «결론어게이트» 8쌍(차단/통과) — 돌연변이 시험이 지킨다.
결론어 = re.compile(
    r"사업|직업|이직|승진|결혼|이혼|합격|취업|유학|이민|부자|가난|성공|실패|"
    r"대박|망한|망해|돈을\s*번|돈을\s*잃|"
    r"좋[다은게아을]|나쁘|나빠|좋아[지진]|"                        # 활용형
    r"길하|길한|길흉|흉하|흉한|길보다|흉보다|흉이\s*크|"           # ⚠「흉」단독은 흉신(정본 노드)이라 안 잡는다
    r"고난|고생|불행|행복|출세|명예|파산|사별|성취|부정적|긍정적|"
    r"잘\s*풀리|안\s*풀리|풀리지\s*않|"
    r"재물이\s*[흩모나들]|돈이\s*[들나모]|"                        # 「재성이 …」는 구조라 통과
    r"건강이\s*[나상악]|병에\s*걸|사고를\s*당")
