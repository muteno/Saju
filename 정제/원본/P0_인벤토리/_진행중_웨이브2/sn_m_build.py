# -*- coding: utf-8 -*-
"""SN-M builder: 중간 JSONL(qline/qslice) -> 최종 posts/paras JSONL.
...
def lines_of(idx):
    f = FILES[idx-1]
    return io.open(f, encoding="utf-8-sig").read().splitlines(), unicodedata.normalize("NFC", os.path.basename(f))
...
def main(src):
    ...
    with io.open(os.path.join(OUT, "SN-M_posts_%s.jsonl" % tag), "w", encoding="utf-8") as fo:
        for p in posts:
            fo.write(json.dumps(p, ensure_ascii=False) + "\n")
    with io.open(os.path.join(OUT, "SN-M_paras_%s.jsonl" % tag), "w", encoding="utf-8") as fo:
        for p in paras:
            fo.write(json.dumps(p, ensure_ascii=False) + "\n")
