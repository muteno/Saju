...
# 계정 체인 SSOT = 워크플로 env 매핑과 동일 순서·순환. 정본 = nomute-editor 이식팩(docs/oauth_승격_타레포_이식팩.md)
#   — 사주 차용(260726): 이 레포 Actions 시크릿 6계정(CLAUDE_CODE_OAUTH_TOKEN_<계정명>)으로 CHAIN만 교체.
#   ⚠️ 체인을 바꾸면 워크플로들의 CLAUDE_CODE_OAUTH_TOKEN_ALT* 조건식도 같이 바꿔야 정합(양쪽 동기 —
#   사주 쪽 완성형 매핑 블록 = docs/oauth_계정로테이션_차용.md).
CHAIN = ["MUTENO", "NOMUTEFB", "EMS1130G", "MUTENONA", "EMS1130M", "EMS1130N"]
THRESHOLD = int(os.environ.get("PROMOTE_THRESHOLD", "2") or "2")   # 활성 계정 쿼터 몇 회 누적 시 승격(운영자 = 2)
API = "https://api.github.com"


def _repo():
    return os.environ.get("GITHUB_REPOSITORY", "muteno/Saju")
