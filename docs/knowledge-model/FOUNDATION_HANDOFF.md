...
갱신: 2026-09-23 · 사용자 원문 [28~42](USER_INTENT_LOG.md)
착수 기준: [PR #187](https://github.com/muteno/Saju/pull/187) 병합 · main `cef1307c673342147afbf74b5996f99a7036864e` · 이번 투간·투출/지장간 검수의 검사·원격 상태는 [체크포인트](WORK_CHECKPOINT.md) 참조
F01 1차 작업 기준: PR #182/main `19dd97f7bd9371975da4e76300d0a72588835a33` · §3 보존 스냅샷 기준: `05a5eab0122eda37235feb66114c6c3d8249a705`
**다음 세션 첫 작업: §5.1의 지장간 겹침4행, 이어 여기·중기·정기 → 월률분야·사령. [투간·투출/지장간 후속](FOUNDATION_EXPOSURE_HIDDEN.md)에서 저자별 정의·자리·시간·구성과 암장 반례를 구분했다. 이번 검색/계산 변경0, 기존 검색 묶음은 의미 동의어 승인이 아니다.**
...
| **F01** | **차이76개·검색 표현4개 적용 / 전체 의미 검수 미완료** | [검토표](FOUNDATION_REVIEW.md)에서 합·충 후속 검토 및 보충 제외는 [후속 문서](FOUNDATION_GENERAL_TERMS.md) 참조. 총칭 실행 등재는 보류하고 [통근 후속](FOUNDATION_ROOTING.md) 원문/가상 측정 완료·추가0. [투간·투출/지장간 후속](FOUNDATION_EXPOSURE_HIDDEN.md) 이후 지장간 겹침4행→여기·중기·정기→월률분야·사령과 기존 별칭의 다른 뜻을 이어 검수. 긴 별칭 추가 시 오행·자리 노드 보존 확인 | 현재67채택(65기존 보존)·5분리·4보류. 상극행은3개 부분 채택/1개 보류. 후속 채택마다 원문·반례·적용 범위·회귀 검사 연결. 승인되지 않은 이름을 실행에 섞지 않음 |
...
### 5.1 다음 세션의 첫 실행 — 지장간 겹침4행 → 여기·중기·정기 → 월률분야·사령

1. `AGENTS.md` 필독 문서 → [체크포인트](WORK_CHECKPOINT.md) → [청사진 v2 §0](FOUNDATION_BLUEPRINT.md)를 읽고 실제 원격 main/PR을 대조한다. [PR #188](https://github.com/muteno/Saju/pull/188)이 미병합이면 검사·검토·병합부터 마무리한다.
2. 먼저 `hidden_overlap_hour_branch`·`hidden_overlap_month_branch` 원문과 측정 JSON의 `current_builder_differences`를 읽는다. 뉴런 검색은 시지장간/연월지장 간에서 시지/월지를 먼저 잡아 지장간4행을 놓친다. 원문2구절의 중복4행이며, 자리와 지장간을 동시에 보존하는 최소 수정을 검토한다. 양쪽 빌더 전체 개념의 전후 ID와 오탐/손실을 비교하고 한쪽 구성 개념을 버리는 긴 별칭으로 덮지 않는다.
3. `foundation_taxonomy_review.json`의 `여기·중기·정기` 행, `candidate_054`, 번들의 `/taxonomy/changed_shared/여기·중기·정기`부터 읽는다. 이번 근거 `hidden_board_chen`·`hidden_hyeonmyo_stages`·`hidden_hyeonmyo_wu_exception`·`hidden_choco_wu`가 출발점이다.
4. 실행 별칭 `여기/중기/정기/본기/正氣`와 두 빌더의 문맥 필터를 대조한다. 일상어 여기·정기적·기본기와 실제 지장간 단계, 같은 문단의 혼합/부정 사례를 구별한다. 기존 필터가 있다는 사실을 의미 정확도 보장으로 읽지 않는다.
5. 다음 현재 노드 `월률분야·사령`의 `월률분야/월령분일용사/사령/당령/인원용사`를 검수한다. 이 노드는 76차이 행에 없으므로 없는 번들 포인터를 만들지 않는다. `사령부/사령관/사령탑` 제외와 월령 문맥을 함께 보고, 지장간 목록·여기/중기/정기·기간 배정·강약 해석을 분리한다.
6. 엔진 정본 `dosa-app/engine/src/tables.js`의 구성/본기 순서와 원문의 기간·비율을 비교하되 계산값을 자동 변경하지 않는다. 절입·시점·월령 범위의 미결은 F03 관법/계산 판본으로 연결한다. 특정 저자의 %를 검증된 확률로 만들지 않는다.
7. 두 빌더에서 같은 입력·개념별 전후 ID·다른 개념 손실 확인 → 단계별 기록·검사·원격 보존 → PR 검사·병합 → 다음 의미 검수/F02 누락31개 연결. 통근·조후의 보류 조건은 그대로 유지한다.

현재 기대값: `foundation_review.py --check`는 **76차이·271개념·적용 별칭4·135근거**, 판정67채택/5분리/4보류. F01 회귀19개. 근거 수는 독립 증거 수가 아니고, 두 행의 검수는 전체 의미 통합이나 앱 관계 계산 완료가 아니다.
...
생성 코퍼스가 없으면 [통근 문서의 생성 명령](FOUNDATION_ROOTING.md#새-체크아웃에서-다시-측정)을 사용한다. 이번 보존/가상 삭제 비교는 `measure_exposure_hidden.py`를 따른다. 과거 측정 스크립트는 고정 기준판/코드 해시를 유지한다. 새로운 실행 변경은 기준·입력·코드 해시를 따로 고정한다.
