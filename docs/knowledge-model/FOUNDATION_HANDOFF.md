...
갱신: 2026-09-23 · 사용자 원문 [28~33](USER_INTENT_LOG.md)
점검 기준: main `05a5eab0122eda37235feb66114c6c3d8249a705`
**현재 할 일: F01 — 로컬/저장소에서 갈라진 사전과 별칭을 대조한다.**
...
| **F01** | **다음 착수** | 273개 이름 후보에서 양쪽 전용 항목·변경된 65개 공통 항목부터 검토. 총칭/하위 개념·다른 뜻·구어 별칭 구별 | 각 차이에 채택/분리/보류와 이유, 대상 파일, 원문 위치를 기록. 승인되지 않은 이름을 실행에 섞지 않음 |
...
**첫 세션의 구체적인 작업:** 기준 커밋/현재 상태를 확인하고 F01 검토표를 만든다. 로컬 총칭 2개와 통근·조후·상극·배우자 구어 별칭을 원문에 대조하고, 적용 가능한 작은 묶음부터 기존 사전·검사 경로에 연결한다. 미확인 항목은 보류 이유를 남기며 앱 계산 변경으로 범위를 넓히지 않는다.
