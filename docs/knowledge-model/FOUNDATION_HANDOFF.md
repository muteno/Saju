...
갱신: 2026-09-23 · 사용자 원문 [28~40](USER_INTENT_LOG.md)
착수 기준: [PR #185](https://github.com/muteno/Saju/pull/185) 병합 · main `ec26ac92834a7df3c21dd6b7a07dd26c85cb6dfc` · 이번 청사진/통근 검수의 원격 상태는 [체크포인트](WORK_CHECKPOINT.md) 참조
F01 1차 작업 기준: PR #182/main `19dd97f7bd9371975da4e76300d0a72588835a33` · §3 보존 스냅샷 기준: `05a5eab0122eda37235feb66114c6c3d8249a705`
**다음 세션 첫 작업: §5.1의 조후 문맥 검수, 이어서 상극. [청사진 v2 §0](FOUNDATION_BLUEPRINT.md)에 실행·보류·복구 기준, [통근 후속](FOUNDATION_ROOTING.md)에 정의 범위3종·별칭6개 보류·측정 결과가 있다. 통근 검색/계산 신규 적용은 0이다.**
...
| **F01** | **차이 76개 1차 처리·표현 1개 적용 / 전체 의미 검수 미완료** | [검토표](FOUNDATION_REVIEW.md)에서 합·충 후속 검토 및 보충 제외는 [후속 문서](FOUNDATION_GENERAL_TERMS.md) 참조. 총칭 실행 등재는 보류하고 [통근 후속](FOUNDATION_ROOTING.md) 원문/가상 측정 완료·추가0. 온도→극 문맥·기존 별칭의 다른 뜻을 이어 검수. 긴 별칭 추가 시 오행·자리 노드 보존 확인 | 1차 판정 66채택(65기존 보존)·5분리·5보류. 후속 채택마다 원문·반례·적용 범위·회귀 검사 연결. 승인되지 않은 이름을 실행에 섞지 않음 |
...
### 5.1 다음 세션의 첫 실행 — 조후 → 상극

1. `AGENTS.md` 필독 문서 → [체크포인트](WORK_CHECKPOINT.md) → [청사진 v2 §0](FOUNDATION_BLUEPRINT.md)를 읽고 실제 main/PR 상태를 확인한다. 통근 후속 PR이 미병합이면 남은 검사·병합부터 마무리한다.
2. `foundation_taxonomy_review.json`의 `조후 균형` 행과 `temperature_positive`·`temperature_weather` 원문을 읽는다. 번들의 `/taxonomy/changed_shared/조후 균형`, 실행 사전의 `한난조습`과 `조후 균형`을 대조한다.
3. 후보 `더위/덥습니다/따뜻한 기운/추위/춥다/춥습니다/한난/한난조습`에서 실제 날씨·계절 비유·명리의 한난조습 언급·균형 판단을 구별한다. 다른 노드에 이미 있는 말을 균형 성립으로 승격하지 않는다.
4. 원문별 채택/보류 근거를 정리하고 안전한 최소 변경만 검토한다. 같은 코퍼스에서 양쪽 빌더의 개념별 전후 ID, 다른 노드 소실을 측정한다. 기준이 부족하면 반례와 해소 조건을 보존하고 상극으로 이동한다.
5. 상극은 `control_positive`·`control_substring`과 `/taxonomy/changed_shared/상극`에서 시작한다. `극합니다`가 `자극합니다`에 포함되는 문제와 `극하는 오행`이 `오행(총칭)`을 가리는 문제를 함께 다룬다. 통근의 긴 별칭 실패를 그대로 반복하지 않는다.
6. 단계별 기록 → 필수 검사 → 검증 단위 원격 저장 → PR 검사·병합 → 다음 다의어/76판정 의미 검수 위치 갱신. D5/P2 생성·재게시와 앱/학습 변경은 별도 검수 범위를 갖출 때 진행한다.

현재 기대값: `foundation_review.py --check`는 **76차이·271개념·적용 별칭1·99근거**, F01 회귀15개. 99개는 구절 위치 수이며 독립 근거 수가 아니다. 통근의 6개 넓은 별칭과 긴 별칭은 미적용이며, 실험 수치는 실제 코드 반영과 구별한다.
...
생성 코퍼스가 없으면 [통근 재현 명령](FOUNDATION_ROOTING.md#새-체크아웃에서-다시-측정)을 사용한다. `measure_rooting.py`는 PR #185와 같은 빌더를 전제로 한 보존 실험이다. 조후/상극 코드를 수정한 뒤 이 기준 검사를 무력화하지 말고 해당 후속 실험의 기준 커밋·해시를 따로 기록한다.
