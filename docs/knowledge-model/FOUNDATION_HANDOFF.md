...
갱신: 2026-09-23 · 사용자 원문 [28~41](USER_INTENT_LOG.md)
착수 기준: [PR #186](https://github.com/muteno/Saju/pull/186) 병합 · main `cacd81db0d3abb1261439b71c7052481c58d8621` · 이번 조후/상극 검수의 검사·원격 상태는 [체크포인트](WORK_CHECKPOINT.md) 참조
F01 1차 작업 기준: PR #182/main `19dd97f7bd9371975da4e76300d0a72588835a33` · §3 보존 스냅샷 기준: `05a5eab0122eda37235feb66114c6c3d8249a705`
**다음 세션 첫 작업: §5.1의 투간·투출 → 지장간 의미 검수. [조후·상극 후속](FOUNDATION_CLIMATE_CONTROL.md)에서 조후8개/상극 긴 표현1개를 보류하고, 짧은 상극3개를 단어 앞 경계 검사와 함께 검색에 반영했다. 검색된 관계의 성립·강도·개인 해석 승인은 아니다.**

2026-09-23 후속: [F01 검토표](FOUNDATION_REVIEW.md)·[판정과 원문 위치](data/foundation_taxonomy_review.json)·[전후 측정](data/foundation_match_measurement.json)을 읽는다. PR #183의 채택66행 중65행은 **기존 판본 보존**이며 새 의미 승인이 아니다. 당시 실제 추가는 `부부 사이`1개였고 이번 상극3개를 포함한 현재 누적은4개다. 부부관계·부부간은 천간합/육합 비유와의 구분 문제로 보류했다. 별칭 판정·P2 재생성 전에 D5 인용 자격을 별도 검수해야 한다. 총칭·넓은 구어와 기존 별칭의 다른 뜻은 남아 있으며 F01 전체 통합 완료로 해석하지 않는다. 아래 §3의 수치는 이전 스냅샷으로 보존한다.
...
| **F01** | **차이76개·검색 표현4개 적용 / 전체 의미 검수 미완료** | [검토표](FOUNDATION_REVIEW.md)에서 합·충 후속 검토 및 보충 제외는 [후속 문서](FOUNDATION_GENERAL_TERMS.md) 참조. 총칭 실행 등재는 보류하고 [통근 후속](FOUNDATION_ROOTING.md) 원문/가상 측정 완료·추가0. [조후/상극 후속](FOUNDATION_CLIMATE_CONTROL.md) 이후 투간·투출→지장간과 기존 별칭의 다른 뜻을 이어 검수. 긴 별칭 추가 시 오행·자리 노드 보존 확인 | 현재67채택(65기존 보존)·5분리·4보류. 상극행은3개 부분 채택/1개 보류. 후속 채택마다 원문·반례·적용 범위·회귀 검사 연결. 승인되지 않은 이름을 실행에 섞지 않음 |
...
### 5.1 다음 세션의 첫 실행 — 투간·투출 → 지장간

1. `AGENTS.md` 필독 문서 → [체크포인트](WORK_CHECKPOINT.md) → [청사진 v2 §0](FOUNDATION_BLUEPRINT.md)를 읽고 실제 원격 main/PR 상태를 대조한다. 조후/상극 PR이 미병합이면 검사·검토·병합부터 마무리한다.
2. `foundation_taxonomy_review.json`의 `투간·투출` 행과 `candidate_081`, 번들의 `/taxonomy/changed_shared/투간·투출`을 읽는다. `기존판 보존`은 의미 검수 완료가 아니다.
3. [통근 후속](FOUNDATION_ROOTING.md)의 `root_choco_same_element`·`root_choco_time_scope`가 가리키는 용어사전 앞뒤를 다시 읽는다. 투간과 투출의 동일 글자/같은 오행, 일간 포함 여부, 월지/동주/거리, 원국/대운·세운 범위가 저자별로 어떻게 다른지 표로 남긴다. 기존 하나의 검색 노드가 의미 동일성을 보장한다고 간주하지 않는다.
4. 다음 `지장간` 행과 `candidate_074`, `/taxonomy/changed_shared/지장간`에서 `地藏干`·`암장` 및 현재 별칭을 검수한다. 글자/장간 구성과 의미 판단을 구별하고 실제 조건은 확인한 정본에서만 가져온다.
5. 최소 검색 변경마다 두 빌더에서 같은 입력 전후 배정과 다른 개념 손실을 확인한다. 의미 분리·새 관계 구조·앱 계산 변경은 원문/정본과 검토 범위를 갖춘 별도 단위로 다룬다. 조후·통근의 보류 조건을 몰래 해소로 바꾸지 않는다.
6. 단계별 기록 → 검사 통과 단위 원격 보존 → PR 검사·병합 → 다음 기존판 보존 항목 지정. 잔여 판정의 의미 검수와 F02 누락31개를 연결한다.

현재 기대값: `foundation_review.py --check`는 **76차이·271개념·적용 별칭4·113근거**, 판정67채택/5분리/4보류. F01 회귀19개. 근거 구간 수는 독립 증거 수가 아니며 상극3개는 문헌 주제 검색만 확장했다.
...
생성 코퍼스가 없으면 [통근 문서의 생성 명령](FOUNDATION_ROOTING.md#새-체크아웃에서-다시-측정)을 사용한다. 최신 비교는 `measure_climate_control.py`를 따른다. 옛 통근 실험은 당시 코드 불변을 전제로 하므로 현재 코드에 맞춰 과거 해시/검사 조건을 낮추지 않는다. 새로운 사전 변경은 해당 후속 비교의 기준·입력·코드 해시를 따로 고정한다.
