...
갱신: 2026-09-23 · 사용자 원문 [28~36](USER_INTENT_LOG.md)
현재 작업 기준: PR #182/main `19dd97f7bd9371975da4e76300d0a72588835a33` · §3 보존 스냅샷 기준: `05a5eab0122eda37235feb66114c6c3d8249a705`
**현재 할 일: F01 잔여 의미 검수 — 76개 차이의 1차 처리와 배우자 검색 표현 1개 적용을 마쳤다.**
...
- **완료:** 지정 로컬 폴더 읽기, 선택 자산 대조, 17개 설계의 반영 점검, 청사진 작성, 이 저장소의 시작 문서들과 검토 자료 연결.
...
- 이 문서 편입은 앱이나 사전 통합 구현이 아니다. 아래 날짜별 기록의 완료 선언·다음 작업·고정 가중치를 현재 상태로 자동 승격하지 않는다.
...
**다음 세션의 구체적인 작업:** 현재 main과 F01 검토표를 확인하고 보류·분리한 항목부터 이어간다. `합을 해→결합을 해`, `충을 해→보충을 해`, `극합니다→자극합니다`의 실물 반례와 통근/기온의 문맥 차이를 보존한다. 별칭 확장 때문에 기존 오행·자리 노드가 가려지지 않는지 함께 검사한다. 미확인 항목은 보류 이유를 남기며 앱 계산 변경으로 범위를 넓히지 않는다.
