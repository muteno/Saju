...
- 월률분야·사령 후속: [검수 결과](FOUNDATION_MONTH_COMMAND.md). 사령부28행 공용 제외·다른 개념 변화0, 사령관/단계/기간 판본은 근거와 재개 조건을 남겨 보류. 근거176구간. 다음은 F02 우선6개이며 기존31개 보고서의 빈도/기각 원칙을 최신 사용자 의도보다 우선하지 않는다.
- 투간·투출→지장간 후속: [검수 결과](FOUNDATION_EXPOSURE_HIDDEN.md). 정의·자리·시간·구성과 비유를 구별해 근거135구간, 이번 검색/계산 변경0. 다음은 새로 확인한 지장간 겹침4행→여기·중기·정기→월률분야·사령. 아래는 이전 실행 이력이다.
- PR #186: 통근 정의 범위3종·뿌리 반례 검수, 넓은6개/긴 표현 추가 보류. 근거99구간.
- 조후→상극 후속: [검수 결과](FOUNDATION_CLIMATE_CONTROL.md). 조후8개·상극 긴1개는 보류하고 짧은3개에 단어 앞 경계를 적용. 근거113·검색 표현4. PR #187 당시 다음 위치는 투간·투출→지장간이었다. 전체 의미 통합·앱 추론·학습은 미완료다.
