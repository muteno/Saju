...
export const Pict = {
   ...
   *
   * ⚠ 260727 재작도 — 선만으로 그린 앞 버전은 19px에서 "뭔지 모르겠다"(운영자)였다. 원인은
   * 이 아이콘만 **윤곽선 도형**이라 작게 줄면 몸통·목·끈이 다 같은 굵기 선으로 뭉갠 것.
   * 그래서 이 하나는 **몸통을 채운다**(fill) — 복주머니는 실루엣이 곧 정체성이라(불룩한 몸통에
   * 잘록한 목) 채워야 19px에서도 한눈에 읽힌다. 위 매듭·끈만 선으로 남겨 '묶은 주머니'가 되고,
   * 배 한가운데 채운 원 하나가 복(福) 자리를 대신한다(글리프 금지 축 — 방식론 §2).
   */
  pouch: (s = 18) => (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.6} strokeLinecap="round" strokeLinejoin="round">
      {/* 몸통 = 채움(잘록한 목 → 불룩한 배 → 둥근 밑). 배를 넓게 잡아야 '주머니'로 읽힌다 */}
      <path
        d="M9.6 9.4h4.8c2.4 1.2 4.4 3.4 4.4 5.9 0 3.3-3 5.4-6.8 5.4s-6.8-2.1-6.8-5.4c0-2.5 2-4.7 4.4-5.9z"
        fill="currentColor"
        stroke="none"
      />
      {/* 주둥이를 묶은 끈 — 몸통 위를 가로지르고 양끝이 아래로 흘러내린다 */}
      <path d="M7.8 9.4h8.4" />
      <path d="M9.8 9.4c-.4.9-1.3 1.5-2.3 1.6M14.2 9.4c.4.9 1.3 1.5 2.3 1.6" />
      {/* 매듭 고 두 개 = 위로 솟은 작은 리본. 크면 선물상자로 읽힌다(260727 실측) */}
      <path d="M12 9.4c-1.1-.5-2.2-.9-2.2-1.8 0-.7.7-1.1 1.3-.8.6.3 .9 1.2 .9 2.6z" />
      <path d="M12 9.4c1.1-.5 2.2-.9 2.2-1.8 0-.7-.7-1.1-1.3-.8-.6.3-.9 1.2-.9 2.6z" />
      {/* 배 한가운데 복(福) 자리 — 채운 몸통에서 도려낸 원이라 작아져도 안 메워진다 */}
      <circle cx="12" cy="15.4" r="1.8" fill="var(--c-ink)" stroke="none" opacity=".5" />
    </svg>
  ),
  /**
   * 사람 바꾸기 — 상담 헤더의 복주머니 **좌측**(운영자 260727 "그 좌측에는 교체 버튼
   * 픽토그램으로 만들어서 누르면 사람 바뀌게"). 형태 = 두 사람이 자리를 맞바꾸는 순환 화살표
   * (사람 머리 둘 + 반대 방향 화살 둘) — 흔한 '새로고침'과 안 헷갈리게 사람을 명시한다.
   */
  swap: (s = 18) => (
    <svg width={s} height={s} viewBox="0 0 24 24" {...svgProps} strokeWidth={1.8}>
      {/* 자리를 비우는 쪽(좌) · 들어오는 쪽(우) */}
      <circle cx="7" cy="6.5" r="2.6" />
      <circle cx="17" cy="17.5" r="2.6" />
      {/* 맞바꿈 = 서로 반대로 도는 화살 둘 */}
      <path d="M13.5 6.5H18l-2-2M18 6.5l-2 2" />
      <path d="M10.5 17.5H6l2-2M6 17.5l2 2" />
    </svg>
  ),
