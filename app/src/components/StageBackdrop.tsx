    ...
    <Box aria-hidden sx={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none' }}>
      {!broken ? (
        <Box
          ...
          // cover = 2:3 원본을 세로 프레임에 맞춰 좌우 기준으로 채운다(위쪽 하늘이 남게 top 정렬).
          // 채도·밝기를 살짝 눌러 유리 판이 앞으로 나오게 한다(사진이 주인공이 아니라 무대라서).
          sx={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center top', filter: 'saturate(.9) brightness(1.04)' }}
        />
      ) : (
